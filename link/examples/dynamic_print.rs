...
fn test_lib(lib: SharedLibrary) {
    ...
    unsafe {
        ...
        let c_str = CString::new("asdf2\n").unwrap().as_ptr(); //.as_bytes().as_ptr();
        let ret: i64 = lib.invoke("printf", (c_str,)).unwrap();
        eprintln!("{:?}", ret);
    }
}
...
fn main() {
    ...
    let musl_n = repo.add_to_new_namespace("musl", musl_path).unwrap();
    let libc_n = repo.add_to_new_namespace("libc", libc_path).unwrap();
